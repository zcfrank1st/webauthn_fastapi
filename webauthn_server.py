...
<div style="display: flex; justify-content: space-around">
  <div style="border: 1px solid black; padding: 10px">
    ...
    <input step="1" id="user-id-register"/>
    <button onclick="register()">Register</button>
  </div>
  <div style="border: 1px solid black; padding: 10px">
    <h2>Authenticate</h2>
    <label for="user-id-auth">User ID:</label>
    <input step="1" id="user-id-auth"/>
...
async def client_js():
    return Path('webauthn_client.js').read_text(encoding="utf-8")


@app.get('/register/{user_id:str}/')
async def register_get(request: Request, user_id: str):
    ...
    print(webauthn.helpers.options_to_json(public_key))
    return json.loads(webauthn.helpers.options_to_json(public_key))
...
@app.get('/auth/{user_id:str}/')
async def auth_get(request: Request, user_id: str):
    try:
